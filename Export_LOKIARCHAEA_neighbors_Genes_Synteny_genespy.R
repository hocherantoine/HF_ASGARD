...
#This is intended to be applied on a per tree basis as the number of clusters / colors is of course limited.
# 
...
#GFF will have been renamed to only contain the assembly name ( GCA_xxx.1.gff)
#Protein names have been reset for compatibility using Fomart_Prodigal_and_Genbank_GFF_to_Genespy

GFFFolder="~/ASGARD/GENOMES/GFF/LOKI/ALL_MOD/"



#Protein names are of the format Assembly$ProteinID
ProteinOfInterestFile="~/ASGARD/HMMSEARCH/PF00808_vs-Asgard_hits_ordered_c__Lokiarchaeia.fa"
...
#Cluster the sequences using mmseqs2 with a very generous setting
...
Clusterstable=read.table("~/ASGARD/SEQUENCES/Gene_Neighboorhoods_Loki/Gene_Neighboorhoods_cluster.tsv",header=F,sep="\t")
...
for(i in GFFFiles){
  ...
  export.gff3(GFF,paste("~/ASGARD/GENOMES/GFF/LOKI/ALL_MOD_Clusters_HistonesSeqs/",i,sep=""))
}
...
LokiHF=read.fasta("~/ASGARD/HMMSEARCH/PF00808_vs-Asgard_hits_ordered_c__Lokiarchaeia.fa")
LokiHFNames=getName(LokiHF)

GenespyInputF="~/ASGARD/ITOL/PF00808_vs-Asgard_hits_ordered_c__Lokiarchaeia_OUTPUT_iTol.txt"
iTOLgenespy=read.delim(GenespyInputF,header=F)
...
for(i in 5:dim(iTOLgenespy)[1]){
  ...
  CorrectedName=LokiHFNames[grep(ProteinName,LokiHFNames)]
...
write.table(iTOLgenespyMod,gsub(".txt","_MOD.txt",GenespyInputF),row.names = F,col.names = F,sep="\t",quote=F)
